...
//    class procedure SetConnectionParam(server, base, user, password: string); static;
    ...
    class function DbConnect2(server, base, user, password: string):boolean; Overload; static;
  end;
...
uses uUtilsRegistry, uDataModule, uSqlList;
...
class function TConn.DbConnect2(server, base, user, password: string): boolean;
begin
  try
    ...
    begin
      if base <> '' then
        FConnection.ExecSQL('if db_id(''' + base + ''') is null create database ' + base + ' COLLATE Latin1_General_CI_AS');


      FConnection.ExecSQL('Use ' + base);

      FConnection.ExecSQL(SqlList['CreateDBSructure']);
    end;
...
begin
  ...
  if ((Server='') or (User='') or (Password='')or (Base='')) then
  begin
    exit(false);
  end;
